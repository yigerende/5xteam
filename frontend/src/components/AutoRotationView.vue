...
<template>
  <section class="page-section auto-rotation-view">
    ...
    <section class="panel list-panel"><div class="panel-title"><div><span>RUN HISTORY</span><h2>轮转执行记录</h2></div><StatusPill tone="pending">{{ runs.length }} 个批次</StatusPill></div><div class="table-shell"><table><thead><tr><th>开始时间</th><th>触发原因</th><th>平均剩余</th><th>席位</th><th>计划/成功/失败</th><th>状态</th><th>操作</th></tr></thead><tbody><tr v-if="!shownRuns.length"><td colspan="7" class="empty-cell">暂无自动轮转记录</td></tr><tr v-for="run in shownRuns" :key="run.id"><td>{{ formatTime(run.started_at) }}</td><td>{{ run.reason || '-' }}</td><td>{{ run.average_percent < 0 ? '未统计' : `${run.average_percent.toFixed(1)}%` }}</td><td>{{ run.seat_remaining }} / {{ run.seat_total }}</td><td>{{ run.planned }} / {{ run.succeeded }} / {{ run.failed }}</td><td><StatusPill :tone="runStatus(run.status)">{{ run.status }}</StatusPill></td><td><button class="btn ghost compact" @click="viewRun(run)"><ChevronRight :size="14" />查看详情</button></td></tr></tbody></table></div><Pagination :page="page" :page-size="pageSize" :total="runs.length" @update:page="page = $event" @update:page-size="pageSize = $event" /></section>
