...
const allSelected = computed(() => accounts.value.length > 0 && accounts.value.every((item) => selected.value.has(item.id)))

async function load() {
  const data = await api(`/api/openai-accounts?page=${page.value}&page_size=${pageSize.value}`)
  accounts.value = data.items || []; total.value = Number(data.total || 0)
  const lastPage = Math.max(1, Math.ceil(total.value / pageSize.value))
  if (page.value > lastPage) { page.value = lastPage; return load() }
  const visible = new Set(accounts.value.map((item) => item.id)); selected.value = new Set([...selected.value].filter((id) => visible.has(id)))
}
async function reload() { await load(); emit('reload') }
function setPage(value) { page.value = value; selected.value = new Set(); load() }
function setPageSize(value) { pageSize.value = value; page.value = 1; selected.value = new Set(); load() }
...
async function save() {
  ...
  form.tokens = ''; form.refreshToken = ''; busy.value = false; await reload()
  setMessage(`已保存 ${saved} 个账号${errors.length ? `，失败 ${errors.length} 个：${errors[0]}` : ''}`, errors.length ? 'error' : 'success')
}
function toggleAll() {
  selected.value = allSelected.value ? new Set() : new Set(accounts.value.map((item) => item.id))
}
function toggle(id) {
  const next = new Set(selected.value); next.has(id) ? next.delete(id) : next.add(id); selected.value = next
}
function targetIDs() { return selected.value.size ? [...selected.value] : [] }
...
async function runBatch(kind) {
  const ids = targetIDs(); if (!total.value) return setMessage('暂无 OpenAI 账号', 'error')
  busy.value = true
  ...
  setMessage(`正在批量${config[0]} ${ids.length || total.value} 个账号...`)
  try {
...
async function remove(account) {
  ...
  try { await api(`/api/openai-accounts/${encodeURIComponent(account.id)}`, { method: 'DELETE' }); selected.value.delete(account.id); await reload(); setMessage(`${account.label} 已删除`, 'success') }
  catch (error) { setMessage(error.message, 'error') }
}
onMounted(load)
</script>
...
<template>
  <section class="view-stack">
    <header class="page-heading"><div><span class="overline">ACCOUNT OPERATIONS</span><h1>OpenAI 账号</h1><p>集中检测凭据状态、刷新令牌并查询可用次数</p></div><StatusPill tone="success">{{ total }} 个账号</StatusPill></header>
